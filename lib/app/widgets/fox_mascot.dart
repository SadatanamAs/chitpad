...
class _FoxPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (showShadow) {
      ...
      canvas.drawOval(
        Rect.fromCenter(center: Offset(cx, h * 0.88), width: w * 0.55, height: h * 0.12),
        shadowPaint,
      );
    }
    ...
    final headRect = RRect.fromRectAndRadius(
      Rect.fromCenter(center: Offset(cx, h * 0.46), width: w * 0.82, height: h * 0.68),
    ...
    final faceRect = RRect.fromRectAndRadius(
      Rect.fromCenter(center: Offset(cx, h * 0.56), width: w * 0.5, height: h * 0.36),
    ...
    canvas.drawOval(
      Rect.fromCenter(center: Offset(w * 0.34, h * 0.38), width: w * 0.16, height: w * 0.16),
      eyeWhitePaint,
    );
    canvas.drawOval(
      Rect.fromCenter(center: Offset(w * 0.66, h * 0.38), width: w * 0.16, height: w * 0.16),
      eyeWhitePaint,
    );
    ...
    canvas.drawOval(
      Rect.fromCenter(center: Offset(cx, h * 0.54), width: w * 0.1, height: w * 0.07),
      nosePaint,
    );
    ...
    if (mood == FoxMood.happy || mood == FoxMood.curious) {
      canvas.drawArc(
        Rect.fromCenter(center: Offset(cx, h * 0.58), width: w * 0.18, height: h * 0.1),
      ...
      canvas.drawArc(
        Rect.fromCenter(center: Offset(cx, h * 0.62), width: w * 0.14, height: h * 0.08),
        2.5, 2.0, false, mouthPaint,
      );
    ...
    if (mood == FoxMood.happy || mood == FoxMood.curious) {
      final blushPaint = Paint()..color = const Color(0xFFFF9A8A).withValues(alpha: 0.35);
      canvas.drawOval(
        Rect.fromCenter(center: Offset(w * 0.22, h * 0.52), width: w * 0.12, height: w * 0.07),
        blushPaint,
      );
      canvas.drawOval(
        Rect.fromCenter(center: Offset(w * 0.78, h * 0.52), width: w * 0.12, height: w * 0.07),
        blushPaint,
      );
    }

    // Eyebrows for curious/thinking/sad
    if (mood == FoxMood.curious || mood == FoxMood.thinking || mood == FoxMood.sad) {
      ...
      if (mood == FoxMood.sad) {
        ...
        canvas.drawLine(Offset(w * 0.26, h * 0.27), Offset(w * 0.38, h * 0.31), browPaint);
        canvas.drawLine(Offset(w * 0.62, h * 0.31), Offset(w * 0.74, h * 0.27), browPaint);
      } else {
        canvas.drawLine(Offset(w * 0.26, h * 0.30), Offset(w * 0.38, h * 0.27), browPaint);
        canvas.drawLine(Offset(w * 0.62, h * 0.27), Offset(w * 0.74, h * 0.30), browPaint);
      }
    }
    ...
    if (mood == FoxMood.waving) {
      ...
      canvas.drawOval(
        Rect.fromCenter(center: Offset(w * 0.82, h * 0.62), width: w * 0.18, height: w * 0.14),
        pawPaint,
      );
    }
  }
