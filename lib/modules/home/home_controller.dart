...
class HomeController extends ChangeNotifier {
  ...
  List<NoteModel> get activeNotes {
    ...
    if (q.isNotEmpty) {
      list = list.where((n) =>
        ...
        n.plainText.toLowerCase().contains(q)).toList();
    }
  ...
  List<Color> get coverColors => isCoverSolid
      ...
      : AppColors.coverGradients[(coverGradientIndex - 12) % AppColors.coverGradients.length];
  ...
  void duplicateNote(String noteId) {
    final note = _notesRepo.getNoteById(noteId);
    if (note != null) {
      _notesRepo.createNote(NoteModel(
        id: '',
